...
const response = await projectRepository.list({ role });
if (!response.success) {
...
try {
   ...
   // Sørg for at `roleIds` alltid er en array
   let roleIds = body.roleIds;

   if (!Array.isArray(roleIds)) {
      // Hvis `roleIds` ikke finnes eller ikke er en array, initialiser som tom array
      roleIds = [];
   } else if (!roleIds.every((id) => typeof id === "number")) {
      // Konverter til `number` hvis nødvendig
      roleIds = roleIds.map((id: any) => Number(id));
   }

   // Kall `create`-metoden fra `projectRepository`
...
if (!project.roles || !Array.isArray(project.roles)) {
   return c.json(
      { success: false, error: "roles must be provided as an array" },
      400
   );
}
